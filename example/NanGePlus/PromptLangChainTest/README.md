...
[B站视频](https://www.bilibili.com/video/BV1zWH1e6ETp/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/mSG7X32CeGA)
(3)【进阶-短期、长期记忆】本系列视频主要介绍prompt工程相关基础及进阶用例，旨在帮助大家更好地了解和使用prompt。一份代码同时支持GPT和各国产大模型
[B站视频](https://www.bilibili.com/video/BV1pWH4eZEN6/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/C2r1bIVJzr0)
(4)【进阶-思维链(Chain of Thoughts CoT) 】本系列视频主要介绍prompt工程相关基础及进阶用例，旨在帮助大家更好地了解和使用prompt
[B站视频](https://www.bilibili.com/video/BV1akpgeeEpG/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/YrOL2eIWw7c)
(5)【进阶-自洽性(Self-Consistency) 】本系列视频主要介绍prompt工程相关基础及进阶用例，旨在帮助大家更好地了解和使用prompt
[B站视频](https://www.bilibili.com/video/BV1d9tKeuEBf/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/Ev-S8AMo_ts)
(6)【进阶-思维树(Tree-of-thought, ToT) 】本系列视频主要介绍prompt工程相关基础及进阶用例，旨在帮助大家更好地了解和使用prompt
[B站视频](https://www.bilibili.com/video/BV1zT4oeSEs1/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/yOtZwClSl_M)
(7)零基础搭建AI Web应用：用Gradio+FastAPI实现前后端分离，一键集成GPT和国产大模型！
[B站视频](https://www.bilibili.com/video/BV1h4xEetEs1/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
[YouTube视频](https://youtu.be/xVG2ru0IEBA)
...
[GitHub地址](https://github.com/NanGePlus/PromptLangchainTest)
[Gitee地址](https://gitee.com/NanGePlus/PromptLangChainTest)
...
进入`basic`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
...
进入`basic`文件夹下，在使用`python apiTest.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数，运行成功后，可以查看smith的跟踪情况
...
进入`withMemoryTest`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
os.environ["LANGCHAIN_TRACING_V2"] = "true"
os.environ["LANGCHAIN_API_KEY"] = "这里填写申请的API_KEY"
**调整2:prompt模版设置相关:**
...
进入`cot`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
os.environ["LANGCHAIN_TRACING_V2"] = "true"
os.environ["LANGCHAIN_API_KEY"] = "这里填写申请的API_KEY"
**调整2:prompt模版设置相关:**
...
进入`selfConsistency`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
os.environ["LANGCHAIN_TRACING_V2"] = "true"
os.environ["LANGCHAIN_API_KEY"] = "这里填写申请的API_KEY"
**调整2:prompt模版设置相关:**
...
进入`tot`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
os.environ["LANGCHAIN_TRACING_V2"] = "true"
os.environ["LANGCHAIN_API_KEY"] = "这里填写申请的API_KEY"
**调整2:prompt模版设置相关:**
...
进入`basicWebUI/gradioTest`文件夹下,先执行`python backend.py`,再执行`python frontend.py`

### （2）在basic应用案例的基础上增加一个WebUI前端页面

1. **启动后端服务**
进入`basicWebUI`文件夹下，在使用`python main.py`命令启动脚本前，需根据自己的实际情况调整代码中的如下参数：
**调整1:设置langsmith环境变量:**
os.environ["LANGCHAIN_TRACING_V2"] = "true"
os.environ["LANGCHAIN_API_KEY"] = "这里填写申请的API_KEY"
**调整2:prompt模版设置相关:**
...
2. **启动前端服务**
**运行apiTest脚本进行检索测试**
进入`basicWebUI`文件夹下，使用`python apiWebUI.py`命令启动前端脚本
**调整1:默认非流式输出 True or False**
stream_flag = False
**调整2:检查URL地址中的IP和PORT是否和main脚本中相同**
url = "http://localhost:8012/v1/chat/completions"
