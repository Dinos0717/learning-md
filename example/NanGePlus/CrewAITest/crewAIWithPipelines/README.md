...
(2) 改写 `crew.py` 代码实现crewai的pipelines功能
...
命令行终端中执行 `cd crewAIWithPipelines` 命令进入到该文件夹内，然后执行如下命令安装依赖包
...
在使用 `python crewPipeline.py` 命令启动脚本前，需根据自己的实际情况调整 `utils/myLLM.py` 代码中参数
