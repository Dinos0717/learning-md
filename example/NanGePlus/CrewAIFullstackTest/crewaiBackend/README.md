...
- [B站视频](https://www.bilibili.com/video/BV1yzHxeZEG5/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
- [YouTube视频](https://youtu.be/w9CRDbafhPI)
...
- [B站视频](https://www.bilibili.com/video/BV1mpH9eVES1/?vd_source=30acb5331e4f5739ebbad50f7cc6b949)
- [YouTube视频](https://youtu.be/thNMan45lWA)
...
- [GitHub](https://github.com/NanGePlus/CrewAIFullstackTest)
- [Gitee](https://gitee.com/NanGePlus/CrewAIFullstackTest)
...
命令行终端中执行 `cd crewaiBackend` 命令进入到该文件夹内，然后执行如下命令安装依赖包
...
在使用 `python main.py` 命令启动脚本前，需根据自己的实际情况调整相关配置参数:
...
在Apifox中新建项目，将提供的 `crewaiBackend` 文件夹下的 `api.json` 接口文件导入
然后，测试运行crew POST请求
...
(4) 引入UI框架
链接地址: [https://element-plus.org/zh-CN/guide/installation.html](https://element-plus.org/zh-CN/guide/installation.html)
...
最后在 `vite.config.js` 中新增内容后重启服务即可
(5) 编写页面代码
...
新增一个组件 `crewaiTest.vue`,代码见 `crewaiTest.vue` 文件
(6) 启动后端服务，前端页面进行测试
测试数据如下:
