...
[Bilibili视频链接](https://www.bilibili.com/video/BV1m89NYKE2J/)

[Youtube链接](https://youtu.be/meuLnVCzEM4)
...
[Bilibili视频链接](https://www.bilibili.com/video/BV1a19bYVEg4/)

[Youtube链接](https://youtu.be/CNmrvtCqveI)
...
[Bilibili视频链接](https://www.bilibili.com/video/BV1cn9mYVENc/)

[Youtube链接](https://youtu.be/4Ok5Ad22yHw)

**(第四期)[2025.3.16]基于LangChain、LangGraph框架和PostgreSQL数据库，结合工具调用和动态路由实现用户意图识别和分诊工作流用例**

**总体流程如下所示:**

[Bilibili视频链接](https://www.bilibili.com/video/BV1AjQvY6Ejv/)

[Youtube链接](https://youtu.be/63wwAy99j54)
...
- 连接池管理：使用 `ConnectionPool` 管理PostgreSQL数据库连接，支持自动提交、超时设置和连接池状态监控（`monitor_connection_pool`）
- 线程内持久化：通过 `PostgresSaver` 保存对话状态检查点
- 跨线程持久化：通过 `PostgresStore` 存储用户记忆（`store_memory`），支持记忆搜索和存储（如用户输入包含"记住"时保存）
- 重试机制：数据库操作使用 `tenacity` 库实现重试（最多3次，指数退避等待），提高健壮性
...
- 消息过滤：`filter_messages` 过滤消息，仅保留 `AIMessage` 和 `HumanMessage`，限制最多保留5条历史消息

**（5）日志与错误处理**

- 日志记录：使用 `logging` 模块和 `ConcurrentRotatingFileHandler` 记录详细日志（DEBUG级别），支持文件轮转（5MB，3个备份）
...
- 可视化：通过 `save_graph_visualization` 将状态图保存为PNG文件，便于调试
- 配置管理：通过 `Config` 类统一管理日志文件路径、数据库URI、提示模板路径等配置
...
[Youtube链接](https://youtu.be/xZQsGN46dUo)

[Bilibili视频链接](https://www.bilibili.com/video/BV1fzXPY7Es7/)
...
官方文档: [https://langchain-ai.github.io/langgraph/](https://langchain-ai.github.io/langgraph/)
...
[Youtube链接](https://youtu.be/KyfGduq5d7w)

[Bilibili视频链接](https://www.bilibili.com/video/BV1nvdpYCE33/)
...
[Youtube链接](https://youtu.be/mTrgVllUl7Y)

[Bilibili视频链接](https://www.bilibili.com/video/BV1BvduYKE75/)
...
[https://github.com/NanGePlus/LangGraphChatBot](https://github.com/NanGePlus/LangGraphChatBot)

[https://gitee.com/NanGePlus/LangGraphChatBot](https://gitee.com/NanGePlus/LangGraphChatBot)
...
`llms.py` 中关于大模型配置参数的调整，以及 `main.py` 脚本中的服务IP和PORT、LangSmith平台的API KEY等的设置

### （2）运行webUI脚本进行测试

进入 `01_ChatBot` 文件夹下，再使用 `python webUI.py` 命令启动脚本前，需根据自己的实际情况调整代码中的如下参数，运行成功后，可以查看smith的跟踪情况

是否要流式输出可设置 `stream_flag = False` 或 `True`，检查URL地址中的IP和PORT是否和main脚本中相同
...
进入 `02_ChatBot` 文件夹下，再使用 `python webUI.py` 命令启动脚本前，需根据自己的实际情况调整代码中的如下参数，运行成功后，可以查看smith的跟踪情况
...
打开命令行终端，`cd 04_ChatBotWithPostgres` 文件夹下，PostgreSQL的docker配置文件为 `docker-compose.yml`
...
进入 `03_ChatBotWithPostgres` 文件夹下，再使用 `python webUI.py` 命令启动脚本前，需根据自己的实际情况调整代码中的如下参数，运行成功后，可以查看smith的跟踪情况
...
[https://github.com/NanGePlus/RagLangChainTest](https://github.com/NanGePlus/RagLangChainTest)
...
经过分析后，`langchain_openai/embeddings` 包中的 `base.py` 源码中默认如下代码的true改为false
...
进入 `04_RagAgent` 文件夹下，再使用 `python webUI.py` 命令启动脚本前，需根据自己的实际情况调整代码中的如下参数，运行成功后，可以查看smith的跟踪情况
